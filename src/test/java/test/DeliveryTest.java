...
public class DeliveryTest {
    ...
    public void testNegativePhone() {
        ...
        $("[data-test-id=\"phone\"] .input__sub").shouldBe(visible).shouldHave(exactText("Поле обязательно для заполнения"));
    ...
    public void testNegativeCity() {
        ...
        $("[data-test-id=\"city\"] .input__sub").shouldBe(visible).shouldHave(exactText("Доставка в выбранный город недоступна"));
    }
    ...
    public void testPositiveReschedule() {
        ...
        $("[data-test-id=\"replan-notification\"]").shouldBe(visible).shouldHave(Condition.text("Необходимо подтверждение У вас уже запланирована встреча на другую дату. Перепланировать?"));
        $(byText("Перепланировать")).click();

        $("[data-test-id=\"success-notification\"]").shouldBe(visible).shouldHave(Condition.text("Успешно! Встреча успешно запланирована на " + firstMeetingDateSeven), Duration.ofSeconds(15));
    }
}
